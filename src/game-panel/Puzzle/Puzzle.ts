...
export class Game{
  ...
  width:number=0;
  height:number=0;
  size:number=1;dL:number=1;loadedImg:number;resourceCount:number;time:number=0;
  ...
  constructor (canvas:any, sound:any=0){
    ...
    this.entitiesImg={
      playArea:{
        self :"./Puzzle/lion.jpg",
      },
      background:{
        self:"./Puzzle/background.png"
      }
    }
  ...
  getSec(){
    return Math.floor(this.time*0.018);
  }
  alert(ctx:any){
    let text="";
    switch(this.state){
      ...
      case STATE.GameOver:text=`Game Over : ${2000-this.getSec()+this.entities.playArea.move}`;break;
      case STATE.Loading:text="Puzzle is Loading....";break;
    }
    ctx.fillStyle="White";
  ...
  constructor (game:Game, canvas:any) {
    ...
    window. addEventListener ("mousedown", (e) =>{
      if(this.game.state==STATE.Running)
        this.game.entities.playArea && this.game.entities.playArea.clickedPiece();
    });
    window. addEventListener ("keydown", (e)=>{ // Cheat Code
      if(e.code=="KeyR" && e.shiftKey)
        ...
        if(this.game.state==STATE.StartScreen){
          this.game.entities.playArea.shuffle();
          this.game.state=STATE.Running;
        }
